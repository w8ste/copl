...
object MutableObjects {
  ...
  ): (List[Location], Store[Value]) = {
    val (argLocations, s3) = exprs.foldLeft((List.empty[Location], store)) { case ((values, s0), expr) =>
      val (v, s1)  = interp(expr, stack.stacked(values.toSet), s0, classes)
  ...
  def interp(
    ...
    expr match {
      ...
      // HINT: be careful that the receiver.fieldValues are “reachable” during any further allocations, you likely need to add them to the stack passed to `allocateAll`
      // Similarly, ensure that any call to malloc includes any newly allocated locations that are not yet included in the current stack.
