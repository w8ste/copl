...
import interpreters.memory.stores.{MarkAndSweepStore, MinimalStore}
...
class MutableObjectsTest extends munit.FunSuite {
  ...
  def evalStore(testclasses: Map[String, Class], e: OOExpr): (Value, MarkAndSweepStore[Value]) = {
    val store = MarkAndSweepStore[Value](
      100,
      {
        case Object(fieldValues = fieldValues) => fieldValues.toSet
        case other                             => Set.empty
      }
    )
    val (v, s) = interp(e, RootedEnv.empty, store, testclassesBase ++ testclasses)
    (v, s.asInstanceOf[MarkAndSweepStore[Value]])
  }
