...
object MarkAndSweepStore {
  ...
  def apply[V](maxSize: Int, locationsInValueFun: V => Set[Location]): MarkAndSweepStore[V] =
    new MarkAndSweepStore(Seq.fill(maxSize)(None), Map.empty, locationsInValueFun)

  def apply[V](size: Int, memory: Seq[Option[V]], locationsInValue: V => Set[Location]) =
    new MarkAndSweepStore[V](memory, Map.empty, locationsInValue)
}
...
) extends VirtualMemoryStore[Val](memory, virtualMemory, locationsInValue) {

  override def withMemory(memory: Seq[Option[Val]], virtual: Map[Location, Location]): MarkAndSweepStore[Val] =
    new MarkAndSweepStore(memory, virtual, locationsInValue)
