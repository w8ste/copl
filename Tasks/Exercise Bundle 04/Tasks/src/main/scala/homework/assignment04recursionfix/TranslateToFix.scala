...
import interpreters.recursion.Box

// ===========================
// Put your changes below here
// ===========================
object TranslateToFixTask {

  import TranslateToFixInterpreter.*
...
// ===========================
// What follows is the standard interpreter. Do not change.
// ===========================

object TranslateToFixInterpreter {
...
// Scala has mutable and immutable maps, this is the common supertype that can store both.
// We use this to mostly keep immutable reasoning, but cheat for recursion and just mutate the map.
    ...
    case App(funExpr, argExpr) =>
      ...
      funV match {
        case Closure(fParam, fBody, fEnv) =>
          interp(fBody, fEnv + (fParam -> Box.Immutable(interp(argExpr, env))))
